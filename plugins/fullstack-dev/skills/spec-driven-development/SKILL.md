...
description: Specifies non-trivial changes before coding. Use when a new project, feature, or significant change has understood requirements but no approved specification.
---
...
Execute tasks one at a time following [`test-driven-development`](../test-driven-development/SKILL.md).
Use [`context-engineering`](../context-engineering/SKILL.md) to load only the approved spec sections
and source files needed for the current task rather than flooding the agent with unrelated context.
