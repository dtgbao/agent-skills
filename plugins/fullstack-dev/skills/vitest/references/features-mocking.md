...
defineConfig({
  test: {
    clearMocks: true,    // Clear before each test (the v5 default)
...
- In v5, `clearMocks` defaults to `true`; set it to `false` only when call history must cross test boundaries
- Keep `vi.mock`, `vi.unmock`, and `vi.hoisted` at module top level; v5 rejects nested calls because they are hoisted anyway
