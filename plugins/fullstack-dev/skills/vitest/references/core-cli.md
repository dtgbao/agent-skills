...
In v5, `-t` matches the full test name with suite and test segments joined by ` > ` (for example,
`vitest -t 'math > adds'`). Match a single segment or use `.*` if the separator should not matter.
