...
In v5, the full name uses ` > ` between suite and test segments. A pattern spanning both segments
must account for that separator, for example `vitest -t 'users > creates a user'`; use `.*` between
segments when the exact separator should not matter.
