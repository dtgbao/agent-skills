...
Colocate everything related to a component/page/feature/module in a single folder. Use the following structure as a guideline:

```
src/{components|pages|features|modules}/
  TaskList/
    TaskListContainer.tsx   # Component container implementation
    TaskList.tsx            # Component implementation
    TaskListItem.tsx        # Component details implementation
    TaskListSkeleton.tsx    # Loading skeleton
    TaskList.test.tsx       # Tests
    TaskList.stories.tsx    # Storybook stories (if using)
    useTaskList.ts          # Custom hook (if complex state)
    lib/
      api.ts                # API calls (if needed)
      constants.ts          # Component-specific constants (if needed)
      types.ts              # Component-specific types (if needed)
      schemas.ts            # Zod schemas for validation (if needed)
      utils.ts              # Component-specific utility functions (if needed)
```
